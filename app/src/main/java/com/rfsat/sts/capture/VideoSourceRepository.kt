...
object VideoSourceRepository {
    ...
    // Renamed from VTB's own store: this app has never written that file, so
    // there is nothing to migrate and a name that lies about which app owns
    // it is a name that misleads whoever reads a backup next.
    private const val PREFS = "sts_video_sources"
