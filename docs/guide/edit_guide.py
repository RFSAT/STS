...
RUN AS
------
    python3 edit_guide.py baseline_v1.58.1.docx STS-User-Guide_v1.49.1.docx
...
EDITS = [
    ...
    # 1.58.1 — the screens now say "scope" where BAS does, so the guide has
    # to name the controls the reader will actually see. Only the UI names
    # and the general term change; "sight radius", "rear sight", "front
    # sight", "iron sight" and the catalogue entries stay, because those name
    # physical parts and BAS keeps them too.
    ("The app reports the running total, the group centre and dispersion, and the sight "
     "correction",
     "The app reports the running total, the group centre and dispersion, and the scope "
     "correction"),

    ("A profile set is your firearm, your load and your sight, kept together. The sight "
     "matters most: it is what turns a group offset in millimetres into a number of "
     "clicks.",
     "A profile set is your firearm, your load and your scope, kept together. The scope "
     "matters most: it is what turns a group offset in millimetres into a number of "
     "clicks."),

    ("Choose a Sight, and check its click value.",
     "Under Optics and Scopes, choose a Scope and check its click value."),

    ("If your sight has no clicks", "If your scope has no clicks"),

    ("The catalogue carries two entries for sights that cannot be clicked: Built-in iron "
     "sight, and No sight.",
     "The catalogue carries two entries for scopes that cannot be clicked: Built-in iron "
     "sight, and No sight."),

    ("With No sight, nothing can be adjusted at all",
     "With No scope, nothing can be adjusted at all"),

    ("Sight correction converts the group offset into clicks for the sight in your "
     "active profile set",
     "Scope correction converts the group offset into clicks for the scope in your "
     "active profile set"),

    ("The shot plot, the score, the group statistics and the sight correction.",
     "The shot plot, the score, the group statistics and the scope correction."),

    ("Sight correction moves the group the wrong way",
     "Scope correction moves the group the wrong way"),

    ("Invert elevation or Invert windage in the sight profile.",
     "Invert elevation or Invert windage in the scope profile."),

    ("The sight correction says no adjustment is needed",
     "The scope correction says no adjustment is needed"),

    ("one click of your sight would move it", "one click of your scope would move it"),
